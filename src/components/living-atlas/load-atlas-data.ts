...
function mapUnmeasuredWatchedRepo(input: {
  ...
  return {
    ...
    name: input.name,
    health: "ok",
    note: `監視中・正規化済み LLM セッション計測なし。${commitNote}`,
    tier: "watched",
    measured: false,
    commitCountToday: input.commitCountToday,
    criteria:
      "判定: 保留。git hook は監視中だが、正規化済み HarnessRun の週次 cache レートがまだ無い。",
    uplift:
  ...
  for (const repoName of measuredRepoNames) {
    ...
    if (rate) {
      ...
      discoveredRows.push({
        ...
        name: repoName,
        health: "ok",
        note: "HarnessRun はあるが、正規化済み usage の週次 cache 率がまだ無い。",
        tier: "discovered",
        measured: false,
        criteria: "判定: 保留。legacy/raw-only 行は cache 率の分母に使わない。",
        uplift: "collector-v3 の正規化済み usage が溜まるまで待つ。backfill は read-only 計画を確認してから行う。",

...
test("A9A-SOURCE-CG5 keeps source preview manual and non-authoritative", async () => {
  ...
  assert.equal(
    packageJson.scripts["harness:evaluate-source-preview"],
    "tsx src/lib/loop-jobs/harness-evaluation/harness-evaluation-source-preview-main.ts",
  );
  assert.equal(
    Object.keys(packageJson.scripts).filter((key) => key.includes("evaluate-source-preview")).length,
    1,
  );

  const cli = await readFile(
    join(root, "src/lib/loop-jobs/harness-evaluation/harness-evaluation-source-preview-cli.ts"),
    "utf8",
  );

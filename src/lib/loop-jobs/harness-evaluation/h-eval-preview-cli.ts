...
export type HEvalPreviewOutputV1 = Readonly<{
  write(line: string, callback: (error: Error | null | undefined) => void): boolean;
}>;

export type HEvalPreviewCliOptionsV1 = Readonly<{
  args: readonly string[];
  input: AsyncIterable<Uint8Array>;
  output: HEvalPreviewOutputV1;
}>;
...
function writeOnePreviewResultV1(output: HEvalPreviewOutputV1, result: PreviewResult): Promise<number> {
...
export async function runHEvalPreviewCliV1(options: HEvalPreviewCliOptionsV1): Promise<number> {
  ...
  if (options.args.length === 0) {
    ...
    if (!bounded.ok) {
      ...
      try {
        result = runHEvalPreviewV1(bounded.value);
      } catch {

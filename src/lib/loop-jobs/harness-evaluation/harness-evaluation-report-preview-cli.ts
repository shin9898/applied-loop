...
export type HarnessEvaluationReportPreviewOutputV1 = Readonly<{
  write(line: string, callback: (error: Error | null | undefined) => void): boolean;
}>;

export type HarnessEvaluationReportPreviewCliOptionsV1 = Readonly<{
  args: readonly string[];
  input: AsyncIterable<Uint8Array>;
  output: HarnessEvaluationReportPreviewOutputV1;
}>;
...
function failureResult(code: PreviewFailureCode): PreviewResult {
  return Object.freeze({
    schema: "harness_evaluation_report_preview_result_v1" as const,
...
/**
 * Explicit stdin-only adapter for an already-reduced evidence envelope.
 * It has no filesystem, database, worker, scheduler, or intervention path.
 */
export async function runHarnessEvaluationReportPreviewCliV1(
  options: HarnessEvaluationReportPreviewCliOptionsV1,
): Promise<number> {
  ...
  if (options.args.length === 0) {
    ...
    } else if (!isHarnessEvaluationEvidenceV1(bounded.value)) {
      result = failureResult("invalid_evidence");
    } else {
      try {
        result = successResult(buildHarnessEvaluationReportV1(bounded.value));
      } catch {

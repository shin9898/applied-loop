...
if (process.env.A8C2_TEST_MODE === "claim-child") {
  ...
  test("A8C2-CG3-T1 scoped one-shot capability remains unreachable by default", async () => {
    ...
    const protectedTreeSha256 = "07151250399a90729c5d84b6436378d8b3c8d0829a6b2e99267bcd4bdcb110ca";
    const allowedPaths = [
    ...
    const inspectImplementationScope = (simulateBaseUnavailable = false) => {
      ...
      if (baseAvailability === "available") {
        ...
        assert.deepEqual(
          ...
          allowedPaths,
          "all historical and C3a/C3p/evaluation-contract/report static compatibility paths must be classified",
        );
    ...
    assert.deepEqual(
      ...
      "fallback exclusion set must remain the exact C2-plus-C3a/C3p/evaluation-contract/report surface",
    );
    assert.equal(
